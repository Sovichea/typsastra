...
- Imported sources without `// @standalone-preview` preview the top-level importer. The first-line directive gives an imported chapter its own preview root; the Preview setting independently selects on-type or on-save refresh.
...
| **Preview Root Files** | Selecting `main.typ` only by filename or coupling root selection to refresh timing. | Build the local import/include graph: imported files use the top-level entry unless their first line is `// @standalone-preview`; select on-type/on-save refresh independently in Settings. | Filename-only routing breaks independent files, while conflating root and refresh policy prevents predictable user control. |
...
| **Template Typography** | Requiring both font roles, using script regex show rules, or writing chapter-only rules that disappear when the chapter is included under a main template. | Use one ordered font fallback stack, keep Latin/complex roles independently optional, and update the local function used by `#show: ...with(...)`. Uniform complex-script scaling is a render-only generated font under `.typstry/fonts/generated`. | Regex show rules reconstruct matched content and break character-level inverse sync. Typography intended for a whole project belongs inside the applied template. Raw content retains Typst's default font behavior. |
