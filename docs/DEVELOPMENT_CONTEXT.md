...
- Language providers are installed globally, then activated per document by the main file's `typsastra:document-languages` directive. Script-aware editing remains independent. Spellcheck, corrections, and typing suggestions are used only when the selected provider advertises the corresponding capability.
...
- The main file's `typsastra:document-languages` directive is the sole language-tool authority. Each script may select one installed provider for spellcheck and completion; unconfigured scripts fail closed. Typst language scopes and keyboard layouts do not reroute providers; see `SCOPE_AWARE_LANGUAGE_TOOLS.md`.
