...
- Language providers are installed globally, then activated per document by the main file's `typsastra:document-scripts` directive. Script-aware editing remains independent. Spellcheck, corrections, and typing suggestions are used only when the selected provider advertises the corresponding capability.
...
- The main file's `typsastra:document-scripts` directive is the sole language-tool authority. Each script may select one installed provider for spellcheck and completion; unconfigured scripts fail closed. Typst language scopes and keyboard layouts do not reroute providers; see `SCOPE_AWARE_LANGUAGE_TOOLS.md`.
- Language provider capabilities use runtime-validated schema version 1. Analysis runs only the one configured provider for each declared script and returns structured per-provider failures without discarding successful results.
...
- Khmer reference fixtures live under `tests/fixtures/khmer/` and lock editing boundaries, deletion, selection, provider tokens, upstream byte spans, CodeMirror UTF-16 ranges, correction spans, and completion replacements to submodule commit `67a79f64f0c68908345099009765615588da1faa` (v0.2.0-rc.2). Run `bun run test:khmer` plus the native `khmer_reference_provider_fixtures_are_locked` test after any related change.
...
- **Core Files**:
  ...
  - `src-tauri/src/font_store.rs`: Current-user font installation, operating-system/private-local font inventory, private-directory collision validation, allowlisted MiSans downloads, and obsolete app-cache migration.
...
- Product terminology: user-facing UI calls an opened directory a **project**. Reserve **workspace** for internal session state, `.typsastra/workspace.json`, file watching, and LSP workspace-root concepts; stable schema fields and backend APIs keep their existing names.
...
- Application defaults live in the platform app-config `settings.json`; preview render/content modes are workspace overrides in `.typsastra/workspace.json`. `typsastra-word-wrap` and `typsastra-theme` localStorage keys are migration inputs only and are removed after the first successful JSON save.
...
- Settings obtains its choices from `list_system_fonts`: code-font options are OS families identified as monospace, while document-text and Unicode fallback choices may use any installed family. `EditorFontManager` maintains separate code and prose stacks with the same complex-script fallbacks.
...
- Imported sources currently preview through the configured main document. Independent standalone roots remain disabled; the portable Full Document/Active File replacement is planned for v0.8.0 and later hardened under `V1X-P.1`. v0.5.3 supports debounced PDF-on-type for short documents and PDF-on-save for long or resource-intensive documents, with non-blocking image-pressure guidance. A bounded SVG renderer is deferred until after v1.0 and remains only a measurement-driven experiment.
...
- **Code vs Prose**: `.cm-content` inherits `--editor-code-font` so incomplete or unclassified Typst syntax remains monospace. Parser tokens carrying `tags.content` override their family with `--editor-text-font` and their optical size with `--editor-text-size`. The default `same-as-code` preference forces an effective scale of `1`, preserving the earlier all-monospace behavior; proportional text defaults to `1.08` and is user-adjustable without changing the shared line height.
- **Semantic Ownership**: Inter-word prose whitespace and visible heading, emphasis, strong, link, and term-list text carry the `content` token. Leading indentation remains untagged and inherits the monospace code font so indentation guides and continuation columns retain stable geometry. Structural tags control weight, scale, style, and decoration but must not override font family. Typst syntax delimiters, strings, comments, math, and raw content remain code-owned.
...
| **Editor and Document Font Roles** | Hardcoding a small selector catalog, exposing arbitrary filesystem fonts to CodeMirror, auto-downloading into app cache, or treating MiSans as mandatory. | Keep source-editor choices limited to OS fonts. For Document Typography, inventory validated global private directories separately, group built-in/private/system families, reject ambiguous family collisions, and pass private paths to every compiler session without copying font binaries. | WebView editor fonts and Typst compiler fonts have different loading boundaries. Private paths are machine-local dependencies, must never enter `.typsastra` or project exports, and require a compiler restart after changes. |
...
| **Template Typography** | Assuming Latin is the base script, forcing strict script coverage on every document, using script show rules, or writing chapter-only rules that disappear under a main template. | Assign each script a font and scale and emit an ordinary ordered Typst fallback stack. Update the local function used by `#show: ...with(...)`. Non-unit scaling uses reusable variants in Typsastra's private global application-data cache; no fonts enter `.typsastra`. | Authors retain familiar, portable fallback order. Strict script ownership remains deferred until shared characters and overlapping font coverage have a predictable model. Project-wide typography belongs inside the applied template. |
