...
- Imported sources currently preview through the configured main document. Independent standalone roots remain disabled; the portable Full Document/Active File replacement is planned for v0.5.3 and later hardened under `V1X-P.1`. v0.5.2 forces PDF-on-save; the on-type setting is retained only for migration and the bounded SVG/PDF hybrid planned for v0.5.3.
...
| **Template Typography** | Assuming Latin is the base script, forcing strict script coverage on every document, using script show rules, or writing chapter-only rules that disappear under a main template. | Assign each script a font and scale, preserve ordinary Typst fallback by default, and emit Unicode `scx` descriptors only when one row explicitly owns `Common` characters. Update the local function used by `#show: ...with(...)`. Non-unit scaling uses reusable variants in Typsastra's private global application-data cache; no fonts enter `.typsastra`. | Authors retain familiar fallback order unless they opt into strict ownership. The optional override prevents cross-script glyph capture without reconstructing content, preserving character-level source sync. Project-wide typography belongs inside the applied template. |
