...
| **Template Typography** | Assuming Latin is always the base script, using script regex show rules, or writing chapter-only rules that disappear when the chapter is included under a main template. | Model one explicit primary script/font plus ordered embedded script fonts, render them as one fallback stack, and update the local function used by `#show: ...with(...)`. Uniform embedded-font scaling is a render-only generated font under `.typsastra/fonts/generated`. | Regex show rules reconstruct matched content and break character-level inverse sync. Typography intended for a whole project belongs inside the applied template. Raw content retains Typst's default font behavior. |
