...
- [ ] **V1X-P.1 Harden portable Active File previews if delivered before or during v1.x.** The implementation was removed from v0.8.0 and has no assigned release. After it lands in a dedicated milestone, qualify its Tinymist task routing, project reference index, placeholder behavior, migrations, source synchronization, Unicode byte offsets, memory bounds, and rapid tab switches against long real-world projects before treating it as a stable contract.
