# Typstry v1.0 Release Implementation Plan

## Objective

Ship Typstry v1.0 as a dependable, complex-script-first research writing environment with reproducible project interchange, guided project creation, and explicit release-quality gates.
...
2. Typst source remains ordinary, portable source. Typstry metadata may guide the application but must never be required by the standard Typst compiler.
...
6. Version-bound means an exact Typst semantic version. Because Typstry manages Tinymist rather than a standalone compiler, the manifest also records the exact Tinymist version that embeds that Typst version.
...
Replace the current unversioned workspace ZIP with a versioned Typstry project archive. Use the extension `.typstry` while retaining ZIP as the underlying container format.

Each archive contains ordinary project files plus a generated manifest at:

```text
.typstry/project.json
```
  ...
  "fonts": [
    {
      ...
      "weight": 400,
      "stretch": 100,
      "path": ".typstry/fonts/package/MiSansKhmer-Regular.ttf",
      "sha256": "...",
      "license": {
        "name": "...",
...
- `.typstry/project.json` and the verified font payload under `.typstry/fonts/package/` are the only generated `.typstry` content exported.
...
```text
Choose or double-click .typstry
...
Export preflight
  ...
  -> copy the exact bytes into .typstry/fonts/package/
...
- If a font cannot be redistributed, block **Export Typstry Project** and offer to select a redistributable replacement. **Export Source ZIP** may remain available but must state that it is not render-reproducible and must not copy a restricted system font.
...
```text
typst compile --font-path .typstry/fonts/package main.typ
```
...
Installed desktop packages should register `.typstry` as **Typstry Project**, with MIME type `application/vnd.typstry.project`, and use the Typstry application icon. The Tauri bundle configuration is expected to declare:
    ...
    "fileAssociations": [
      {
        ...
        "description": "Version-bound Typstry project archive",
        "mimeType": "application/vnd.typstry.project",
        "role": "Editor",
        "rank": "Owner",
        "exportedType": {
          "identifier": "com.typstry.project",
...
```text
Operating system opens project.typstry
  -> Typstry receives and canonicalizes the file path
...
Cold launch and already-running behavior must both work. When Typstry is already open, a single-instance handoff should deliver the path to the existing window, focus it, and open one import dialog. Repeated OS events for the same canonical path must be deduplicated. Unsupported/corrupt files receive an error without changing the active workspace.

Installer verification is required for Windows MSI/NSIS, Linux DEB/RPM desktop integration, and macOS application bundles. AppImage association depends on the user's desktop integration mechanism, so Typstry must not claim automatic association for a standalone AppImage.
...
- [ ] **V1-I.14 Test the interchange contract.** Cover Unicode paths, large projects, corrupt ZIPs, zip-slip, hash mismatch, missing main files, unavailable versions, cancellation, disk-full behavior, and Windows/Linux/macOS round trips.
- [ ] **V1-I.15 Register the file association.** Add `.typstry`, `application/vnd.typstry.project`, the exported macOS type, the Typstry icon, and installer metadata without claiming ownership of `.typ` or `.typst` source files.
- [ ] **V1-I.16 Route OS-open events safely.** Handle cold launch and single-instance handoff, queue requests until initialization completes, canonicalize and deduplicate paths, focus the existing window, and invoke the same import controller used by the File menu.
- [ ] **V1-I.17 Test packaged double-click import.** Verify association, icon, cold/warm launch, spaces and Unicode paths, corrupt archives, repeated events, cancellation, and uninstall cleanup on every supported installer format.
...
- [ ] Double-clicking a `.typstry` file opens one validated import flow in either a new or already-running Typstry instance.
- [ ] A `.typstry` export contains every exact font face used by its validated render or fails with an actionable font/license report.
...
- [ ] Ordinary `.typ` sources still compile outside Typstry.
...
Start writing with Typstry and Typst.
```
...
- [ ] Generated projects are ordinary Typst projects and do not require Typstry to compile.
...
Typstry is eligible for v1.0 only when:
...
- [Typstry v1.x implementation plan](./V1X_IMPLEMENTATION_PLAN.md)
- [Typstry v2 implementation plan](./V2_IMPLEMENTATION_PLAN.md)
