...
pub async fn install_unicode_font(font_id: &str) -> Result<InstalledFont, String> {
    if let Some(spec) = DIRECT_DOWNLOADS.iter().find(|spec| spec.id == font_id) {
        ...
        let installed_file = format!("Typstry-{}", spec.file_name);
        write_and_register(
            &installed_file,
    ...
    let installed_regular = format!("Typstry-{}", spec.regular_file);
    let installed_bold = format!("Typstry-{}", spec.bold_file);
    write_and_register(
        &installed_regular,
