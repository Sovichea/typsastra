...
pub fn list_hunspell_catalog(
    ...
    let mut entries = vec![HunspellCatalogEntry {
        ...
        installed: true,
        bundled: true,
        source: "Bundled with Typsastra".to_string(),
...
pub async fn install_hunspell_dictionary(
    ...
    let spec = find_hunspell_spec(&locale)
        .ok_or_else(|| format!("{locale} is not in Typsastra's Hunspell catalog."))?;
...
mod license_tests {
    ...
    /// Every provider that ships with Typsastra must declare a known, redistributable license.
